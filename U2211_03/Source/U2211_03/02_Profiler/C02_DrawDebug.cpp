...
void AC02_DrawDebug::Tick(float DeltaTime)
{
	...
	location.Z += EaseOutBounce(FMath::Sin(GetWorld()->GetTimeSeconds() * 3.0f)) * 400.0f;

	DrawDebugCapsule(GetWorld(), location, 200, 50, FQuat::Identity, FColor::White);
	DrawDebugDirectionalArrow(GetWorld(), Location[3], location, 400, FColor::Black, false, -1, 0, 30);
